...
print(students.to_string(index=False,header=False),"\n")

print("Long Format Dataset")
long_form = pd.melt(students, id_vars=["StudentID","StudentName"],var_name="Subject",value_name="Marks")
print(long_form.to_string(index=False,header=False),"\n")

print("Reconstructed Dataset")
wide_form = long_form.pivot(index=["StudentID","StudentName"], values="Marks", columns="Subject").reset_index()
print(wide_form[["StudentID","StudentName","Math","Science","English"]].to_string(index=False,header=False))