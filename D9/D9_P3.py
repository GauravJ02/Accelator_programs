...
print("Sales Dataset")
print(sales.to_string(index=False,header=False),"\n")

print("Department-wise Monthly Sales","\n")

pivot = sales.pivot_table(values="Sales",index="Department",columns="Month",aggfunc="sum",fill_value=0)
print(pivot,"\n")

print("Highest Revenue Department","\n")
print(pivot.sum(axis=1).idxmax(),pivot.sum(axis=1).max(),"\n")

print("Highest Revenue Month","\n")
print(pivot.sum(axis=0).idxmax(),pivot.sum(axis=0).max())