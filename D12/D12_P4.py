...
maximum = []
for chunk in pd.read_csv("metrics.csv",chunksize=2,usecols=["MetricValue"]):
    chunk["MetricValue"] = pd.to_numeric(chunk["MetricValue"],downcast="float")
    print("Chunk Processed - Overhead:",chunk["MetricValue"].memory_usage(index=False),"Bytes")
    maximum.append(chunk["MetricValue"].max())
    
print("Global Peak Metric Observed:",max(maximum))