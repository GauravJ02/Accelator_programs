...
system_logs['MemoryUsagePct'] = pd.to_numeric(system_logs['MemoryUsagePct'],downcast='float')

pl_df = pl.from_pandas(system_logs).lazy()

result = (pl_df.filter(pl.col('MemoryUsagePct') > 75.0))

print("Pandas Vector Optimized Type: ",system_logs['MemoryUsagePct'].dtype)
print("High Resource Violations Detected:")
for row in result.collect().iter_rows():
    print(*row)