...
report = orders.merge(customers,on="CustomerID").merge(products,on="ProductID").merge(payments,on="OrderID")
print("Complete Sales Report")
print(report[["OrderID","CustomerName","City","ProductName","Category","Quantity","PaymentAmount"]].to_string(header=False,index=False),"\n")

print("Electronics Orders")
electronics = report.loc[report["Category"] == "Electronics"]
print(electronics[["OrderID","CustomerName","ProductName","PaymentAmount"]].to_string(header=False,index=False),"\n")

print("Second and Third Orders")
print(report[["OrderID","CustomerName","ProductName","PaymentAmount"]].iloc[1:3].to_string(header=False,index=False),"\n")

print("Category Revenue Report")
data = report.groupby("Category")["PaymentAmount"].sum()
print(data.sort_values(ascending=False).to_string(header=False),"\n")

print("Highest Revenue Category")
print(f"{data.idxmax()} {data.max()}","\n")

print("Highest Spending Customer")
customer = report.groupby("CustomerName")["PaymentAmount"].sum()
