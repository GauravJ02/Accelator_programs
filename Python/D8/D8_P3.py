...
print(patients.to_string(index=False, header=False),"\n")

print("Department Statistics")
stats = patients.groupby("Department")["TreatmentCost"].agg(["sum", "mean", "max", "min"])
stats.index.name = None
print(stats.to_string(header=False),"\n")

sorted_df = stats.sort_values("sum")
print("High Cost Departments")
filtered = stats[stats["mean"] >= 50000]
print(filtered.loc[:,"mean"].to_string(header=False))